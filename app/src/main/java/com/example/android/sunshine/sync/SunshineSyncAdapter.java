...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    private void updateWatchFace(Context context) {
        ...
        if (cursor.moveToFirst()) {
            ...
            String temperatureData = String.valueOf(cursor.getDouble(INDEX_MAX_TEMP) + " " + cursor.getDouble(INDEX_MIN_TEMP));

            PutDataMapRequest dataMap = PutDataMapRequest.create(WEATHER_DATA_URI);
            dataMap.getDataMap().putString(WEATHER_DATA, temperatureData);
        ...
        if (displayNotifications) {
            ...
            if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS) {
                ...
                if (cursor.moveToFirst()) {
                    ...
                    int artResourceId = SunshineWeatherUtils.getLargeArtResourceIdForWeatherCondition(weatherId);
                    String artUrl = SunshineWeatherUtils.getArtUrlForWeatherCondition(context, weatherId);
    ...
    static private void setLocationStatus(Context c, @LocationStatus int locationStatus) {
        ...
        spe.putInt(c.getString(R.string.pref_location_key), locationStatus);
        spe.commit();
    }
}